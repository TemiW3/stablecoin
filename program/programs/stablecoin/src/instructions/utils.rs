...
// Add the correct import for PriceUpdateV2
use crate::{constants::{FEED_ID, MAX_AGE, PRICE_FEED_DECIMAL_ADJUSTMENT}, state::{Collateral, Config}, StablecoinError};
...
pub fn check_health_factor(
    ...
    require!(health_factor >= config.minimum_health_factor, StablecoinError::BelowMinHealthFactor);
...
pub fn get_usd_value(amount_in_lamports: &u64, price_feed: &Account<PriceUpdateV2>) -> Result<u64> {
    // Assuming price_feed contains the price in USD per lamport
